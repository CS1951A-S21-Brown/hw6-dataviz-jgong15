let topGames_width = MAX_WIDTH/2 - 20, topGames_height = 350;
...
let x = d3.scaleLinear()
    .range([0, topGames_width - margin.left - margin.right]);
...
let y_axis_text = svg.append("text")
    .attr("transform", `translate(${-120} , ${(topGames_height - margin.top - margin.bottom)/2})`)       // HINT: Place this at the center left edge of the graph
    .style("text-anchor", "middle");
...
function setData_topGames(year) {
    ...
    update_yearSelect(years, "topGamesSelect");

    data = filterYear(data, year, "Year");
